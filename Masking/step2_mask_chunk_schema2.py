...
def process_input_table_page(
    ...
    rows = jamai.table.list_table_rows(
        ...
        columns=["file_path", "Transcription", "Segments", "SegmentsAndWords", "Masking_need"],
        limit=limit,
        offset=offset
    )
    ...
    for row_data in rows_with_card_info:
        ...
        segments_json = json.loads(row_data.get("Segments") or "")
        segments_word_json = str(row_data.get("SegmentsAndWords") or "")
        
        chunks = split_segments_into_chunks(segments_word_json, token_limit, tokenizer)
        ...
        for i, chunk_data in enumerate(chunks, 1):
            insert_payload = {
                ...
                "Segments": json.dumps(segments_json, ensure_ascii=False) if segments_json else "[]",
